  ...
  function formatHelpReport(session) {
    ...
    if (language === 'en') {
      return [
        ...
        !botProvider ? '• `!provider <codex|claude|antigravity|status>` — switch provider for current channel; `gemini` is accepted as a legacy alias' : null,
        '',
        '**Workspace**',
    ...
    return [
      ...
      !botProvider ? '• `!provider <codex|claude|antigravity|status>` — 切换当前频道 provider；`gemini` 是 legacy alias' : null,
      '',
      '**工作目录**',
