...
代码仓库放在内部 Gitea：`https://git.talesofai.com/atou/external-signal-material-pool`。

第一版技术栈采用 Python 3.12+。原因是它适合做文件式 CLI、RSS/XML 解析、JSONL 协议校验、第三方命令封装和 fixture 测试。CLI 可以用 Typer，schema 校验用 Pydantic，测试用 pytest。后续如果要服务化，再在同一协议上包 API，不改变材料池文件格式。
